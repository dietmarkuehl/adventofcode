...
int main() {
    ...
    while (not queue.empty())
    {
        ...
        if (0u < row && dir != direction::down && (dir != direction::up || straight < 3u))
            queue.emplace(row - 1, column, cost,
                          dir == direction::up? straight + 1u: 1u, direction::up);
        if (0u < column && dir != direction::right && (dir != direction::left || straight < 3u))
            queue.emplace(row, column - 1, cost,
                          dir == direction::left? straight + 1u: 1u, direction::left);
        if (row < map.size() - 1 && dir != direction::up && (dir != direction::down || straight < 3u))
            queue.emplace(row + 1, column, cost,
                          dir == direction::down? straight + 1u: 1u, direction::down);
        if (column < map[0].size() - 1
            && dir != direction::left && (dir != direction::right || straight < 3u))
            queue.emplace(row, column + 1, cost,

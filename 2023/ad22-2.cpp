...
int main() {
    ...
    {
        brick const* b;
        bool operator< (brick_ref const& o) const { return this->b->c[0].z < o.b->c[0].z; }
    };
    for (auto const& b: bricks)
    {
        std::priority_queue<brick_ref>   queue;
        std::unordered_set<brick const*> d;
        queue.emplace(&b);
        d.insert(&b);
        while (!queue.empty())
        {
            brick const& n{*queue.top().b};
            queue.pop();
            for (auto const* s: n.supporting)
            {
                std::size_t sc{s->supported.size()};
                for (auto const* p: s->supported)
                {
                    if (d.find(p) != d.end())
                        --sc;
                }

...
int brute(node const& n, int time, std::vector<bool> open, std::vector<bool> visited, int np, int limit) {
    if (29 < time || limit <= np) {
        return 0;
    }
    ...
    for (auto const& t: n.tunnels) {
         if (!visited[t.valve->index()]) {
             max = std::max(max, brute(*t.valve, time + t.cost, open, visited, np + 1, limit));
         }
    }
    if (!open[n.index()]) {
        ...
        open[n.index()] = true;
        for (auto const& t: n.tunnels) {
            max = std::max(max, flow + brute(*t.valve, time + t.cost + 1, open, std::vector<bool>(visited.size(), false), 0, limit + 1));
        }
    }
...
int main(int ac, char* av[]) {
    ...
    open[g.get_node("AA")->index()] = true;
    auto total = brute(*g.get_node("AA"), 1, open, unset, 0, 2);
